(function (global) {
  ...
  function communityNav(on, meId) {
    ...
    return `<nav class="subnav">
      <a class="${on === "feed" ? "is-on" : ""}" href="/community">Feed</a>
      <a class="${on === "people" ? "is-on" : ""}" href="/people">People</a>
      ${meId ? `<a class="${on === "profile" ? "is-on" : ""}" href="/profile/${meId}">Your profile</a>` : ""}
    </nav>`;
  }
  ...
  function accountNav(on) {
    ...
    return `<nav class="subnav">${links.map(([h, l, p]) =>
      `<a class="${on === p ? "is-on" : ""}" href="${h}">${l}</a>`).join("")}</nav>`;
  }
