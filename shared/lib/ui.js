(function (global) {
  ...
  function icon(name, opts) {
    ...
    return `<span class="ico" data-ico="${esc(name)}" data-glyph="${g}" aria-hidden="true"><svg class="ico-svg" width="16" height="16" viewBox="0 0 24 24" fill="${fill}" stroke="currentColor" stroke-width="1.75" stroke-linecap="round" stroke-linejoin="round">${d}</svg></span>`;
  }
  function stars(n) {
  ...
  function actionPills(p) {
    const id = Number(p.id);
    const wished = lists.hasWish(id);
    const compared = lists.hasCompare(id);
    const inCart = lists.inCart(id);
    return `<button type="button" class="pill${wished ? " is-on" : ""}" data-wish="${id}" aria-pressed="${wished}">${icon("save", { filled: wished })} ${wished ? "Unsave" : "Save"}</button>
          <button type="button" class="pill${compared ? " is-on" : ""}" data-compare="${id}" aria-pressed="${compared}">${icon("compare")} ${compared ? "Compared" : "Compare"}</button>
          <button type="button" class="pill${inCart ? " is-on" : ""}" data-cart="${id}" aria-pressed="${inCart}">${icon(inCart ? "check" : "add")} ${inCart ? "In cart" : "Add"}</button>`;
  }

  function paintAction(btn, kind, on) {
    if (!btn) return;
    btn.classList.toggle("is-on", !!on);
    btn.setAttribute("aria-pressed", on ? "true" : "false");
    const id = btn.dataset.wish || btn.dataset.compare || btn.dataset.cart;
    if (kind === "wish") btn.innerHTML = `${icon("save", { filled: on })} ${on ? "Unsave" : "Save"}`;
    else if (kind === "compare") btn.innerHTML = `${icon("compare")} ${on ? "Compared" : "Compare"}`;
    else btn.innerHTML = `${icon(on ? "check" : "add")} ${on ? "In cart" : "Add"}`;
    if (id) {
      if (kind === "wish") btn.dataset.wish = id;
      if (kind === "compare") btn.dataset.compare = id;
      if (kind === "cart") btn.dataset.cart = id;
    }
  }
    ...
    el.addEventListener("click", async (e) => {
      ...
      try {
        ...
        if (cart) {
          ...
          const on = !!(res && res.added);
          lists.setCart(id, on, res && res.item_id);
          if (on) await lists.refresh();
          paintAction(cart, "cart", lists.inCart(id));
          toast(on ? "Added to cart" : "Removed from cart");
        }
  ...
  function syncBuyButtons(p) {
    ...
    if (add) {
      ...
      add.setAttribute("aria-pressed", inCart ? "true" : "false");
    }
    if (wish) {
      wish.textContent = wished ? "Unsave" : "Save to wishlist";
      wish.classList.toggle("is-on", wished);
      wish.setAttribute("aria-pressed", wished ? "true" : "false");
    }
    if (cmp) {
