...
Remaining work to match `docs/PRODUCT-SPEC.md`. When this file has no `### FIX-` sections, the queue is done.

HTTP 200 on HTML is not proof that an item is done. Hosts stay in `config/site.env`.
...
- Public and service HTTP paths are `/api/v1/...`.
...
- **Expected:** payment details include destination, `crypto_chain`, `crypto_amount` ≈ `total_usd / rate_to_usd` for the chosen chain. Placeholder `0xSTORE4BOTS_WALLET_BSC` is not acceptable as the only production address — use settings `payment_address`.
...
- **Spec:** §5.8
- **Expected:** `GET /sitemap.xml` is XML urlset (products, categories, bundles, posts). `GET /robots.txt` is a robots file. Neither is `<title>Store4bots — Digital Assets Marketplace</title>`.
...
- **Observed:** `/post/1` and `/profile/1` returned title “Store4bots — Community” and the same ~12kB feed shell.
...
- **Surface:** identity-service
- **Spec:** §4.2
- **Expected:** `POST /api/v1/register` 201 with `{ token, user }`. `Set-Cookie` includes `store4bots_session` (HttpOnly). Duplicate email → 409. Missing/invalid email or short password → 400.
...
- **Surface:** identity-service
- **Blocked by:** FIX-034
- **Expected:** `POST /logout` `Set-Cookie` expires `store4bots_session`. Browser with only that cookie then gets 401 on `/me`.
...
- **Observed:** `Store4bots.api.products.list()` is the client; raw `api.get('/products')` is not.
