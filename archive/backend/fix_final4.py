#!/usr/bin/env python3
"""Fix double-quoted mock strings -> backtick, correctly. Round 3."""
import re
...
for line in text.split('\n'):
    ...
    if idx_q >= 0 and (idx_e < 0 or idx_q <= idx_e):
        method = "ExpectQuery"
        start_marker = 'mock.ExpectQuery("'
        start = idx_q
    else:
        method = "ExpectExec"
        start_marker = 'mock.ExpectExec("'
        start = idx_e
    
    q_open = start + len(start_marker) - 1  # position of opening "
    i = q_open + 1
    parts = []
    while i < len(s):
        c = s[i]
