#!/usr/bin/env python3
"""Fix double-quoted mock strings -> backtick, correctly handling escapes."""
...
for line in text.split('\n'):
    ...
    while i < len(s):
        ...
        if c == '\\' and i + 1 < len(s):
            nxt = s[i+1]
            if nxt == '\\':
                # \\ in source -> one \ in backtick output
                # But wait: in Go double-quoted, \\\\ = \ (one backslash)
                # In the FILE BYTES, we see \\ which Go interprets as \
                # So from the file's perspective, \\ in the source = \ in the string value
                # For backtick, we want the string value: \ (one backslash)
                parts.append('\\')
                i += 2
            elif nxt == '$':
                # \$ in source -> Go sees invalid escape, but file intends \$ (regex escape for $)
                # In backtick: \$ (literal backslash + dollar)
                parts.append('\\$')
                i += 2
            elif nxt == '(':
                parts.append'\\(')
