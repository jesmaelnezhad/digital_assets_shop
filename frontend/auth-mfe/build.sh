#!/usr/bin/env bash
set -euo pipefail
cd /root/project/frontend/auth-mfe
...
cp src/login.html dist/ 2>/dev/null || true
cp src/register.html dist/ 2>/dev/null || true
...
docker build -t 130.185.123.156:30099/pawradise/auth-mfe:latest .
echo "Built auth-mfe"