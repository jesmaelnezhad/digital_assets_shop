#!/usr/bin/env bash
set -euo pipefail
cd /root/project/frontend/checkout-mfe
rm -rf dist && mkdir -p dist/checkout dist/assets
cp src/assets/favicon.svg dist/assets/ 2>/dev/null || true
cp src/index.html dist/ 2>/dev/null || true
cp src/checkout.html dist/ 2>/dev/null || true
cp src/checkout.html dist/checkout/checkout.html 2>/dev/null || true
...
docker build -t 130.185.123.156:30099/pawradise/checkout-mfe:latest .
echo "Built checkout-mfe"