...
test.describe('7. Frontend-API Integration', () => {
  test('api.products.list() works', async ({ page }) => {
    ...
    const data = await page.evaluate(async () => {
      return await window.Store4bots.api.products.list({ per_page: 2 });
    });
  ...
  test('api.products.getCategories() works', async ({ page }) => {
    ...
    await page.waitForTimeout(2000);
    const data = await page.evaluate(async () => {
      return await window.Store4bots.api.products.getCategories();
    });
  ...
  test('api.products.get(slug) works', async ({ page }) => {
    ...
    await page.waitForTimeout(2000);
    const data = await page.evaluate(async () => {
      return await window.Store4bots.api.products.get('product-1');
    });
  ...
  test('api.community.getPosts() works', async ({ page }) => {
    ...
    await page.waitForTimeout(2000);
    const data = await page.evaluate(async () => {
      return await window.Store4bots.api.community.getPosts('recent', 1, 5);
    });
